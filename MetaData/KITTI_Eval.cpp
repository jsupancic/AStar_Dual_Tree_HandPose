...
void cleanData(CLASSES current_class, const vector<tGroundtruth> &gt, const vector<tDetection> &det, vector<int32_t> &ignored_gt, vector<tGroundtruth> &dc, vector<int32_t> &ignored_det, int32_t &n_gt, DIFFICULTY difficulty){
  ...
  for(int32_t i=0;i<gt.size(); i++){
    ...
    string gt_type = gt[i].box.type;
    string dt_type = CLASS_NAMES[current_class];
    cout << gt_type << " ?=? " << dt_type << endl;
    if(!strcasecmp(gt_type.c_str(), dt_type.c_str()))
      valid_class = 1;
...
tPrData computeStatistics(
  ...
  const vector<tGroundtruth> &dc,
  const vector<int32_t> &ignored_gt,
  const vector<int32_t>  &ignored_det,
  bool compute_fp,
  ...
  vector<double> &aos, DIFFICULTY difficulty) 
{
  ...
  double ap = mean(precision);
  log_once(printfpp("AP (%s) = %f",CLASS_NAMES[current_class].c_str(),ap));
  return true;
}
...
void saveAndPlotPlots(string dir_name,string file_name,string obj_type,vector<double> vals[],bool is_aos){
  ...
  assert(system(command) == RET_SUCCESS);
...
bool eval(){
  ...
  string gt_dir         = params::KITTI_dir() + "/object/training/label_2";
  string result_dir     = g_params.require("RESULTS");
  string plot_dir       = params::out_dir() + "./plots/";

  // create output directories
  do_command(("mkdir -p " + plot_dir).c_str());
  ...
  for (int32_t i=0; i<N_TESTIMAGES; i++) {
    ...
    string gt_filename = gt_dir + "/" + file_name;
    vector<tGroundtruth> gt   = loadGroundtruth(gt_filename.c_str(),gt_success);
    string rt_filename = result_dir + file_name;
    vector<tDetection>   det  = loadDetections(rt_filename.c_str(), compute_aos, eval_car, eval_pedestrian, eval_cyclist,det_success);

    // check for errors
    if (!gt_success) {
      cout << 
      printfpp("ERROR: Couldn't read: %s of ground truth. Please write me an email!",
	       gt_filename.c_str()) << endl;
      return false;
    }
    else if (!det_success) {
      cout << printfpp("warning: Couldn't read: %s", rt_filename.c_str()) << endl;
    }
    else
    {
      cout << printfpp("notice: accepted: %s", rt_filename.c_str()) << endl;
